        ...
        if (track.spurroad.size() >= 1) //岔路标志
        {
            ...
            if (freezoneStep == FreezoneStep::FreezoneEntering || freezoneStep == FreezoneStep::FreezoneExiting)
            {
                ...
                if (directionLeft) //选择左入泛行区
                {
                    ...
                    _index = "2";
                    if (track.pointsEdgeRight[rowBreakLeft].x < track.spurroad[indexSpurroad].x)
                    {
                        _index = "3";
                        repairEnable = true;
                    }
                    ...
                    {
                        if (rowBreakRight < track.pointsEdgeRight.size()) //由左边缘补偿点映射右边缘补偿点
                        {
                            rowBreakLeft = rowBreakRight;
                            if (track.spurroad[indexSpurroad].x != track.pointsEdgeRight[rowBreakLeft].x)
                            {
                                _index = "4";
                                repairEnable = true;
                            }
                    ...
                    {
                        uint16_t rowEnd = rowBreakLeft;
                        for (int i = rowBreakLeft; i < track.pointsEdgeLeft.size(); i++)
                        {
                            if (track.pointsEdgeLeft[i].x <= track.spurroad[indexSpurroad].x)
                            {
                                rowEnd = i;
                                break;
                            }
                        }
                        if (rowEnd != rowBreakLeft)
                        {
                            double k = (double)(track.spurroad[indexSpurroad].y - track.pointsEdgeLeft[rowBreakLeft].y) /
                                       (double)(track.spurroad[indexSpurroad].x - track.pointsEdgeLeft[rowBreakLeft].x);
                            double b = track.spurroad[indexSpurroad].y - k * track.spurroad[indexSpurroad].x;
                            for (int i = rowBreakLeft; i <= rowEnd; i++)
                            {
