...
public:
  ...
  bool startingCheck(vector<PredictResult> predict)
  {
    if (startingFilt)
    {
      ...
      if (counterCrosswalk)
      {
        ...
        if (counterCrosswalk > 2 && counterSessionTwo < 12)
        {
          counterSessionTwo = 0;
          counterCrosswalk = 0;
          return true;
  ...
  void garageEntryRecognition(TrackRecognition& track, vector<PredictResult> predict)
  {
      ...
      if (counterRec >= 1 && garageStep == GarageEntryRecognition) //通过AI标志提前识别车库|避免误识别环岛
      {
          garageStep = GarageEntryingA;
      }

      if (counterRec >= 1 && crosswalk.x > ROWSIMAGE / 8) // AI识别到斑马线：减速，精确识别入库图像
      {
          slowDown = true;
      }
    ...
    //          if (track.pointsEdgeLeft[rowBreakLeftUp].y > COLSIMAGE / 2 - 40)
    //          {
