...
public:
    ...
    vector<int> Numofblock;
    vector<int> Widthofblack;
    ...
    uint16_t rowCutUp = 50;             //图像顶部切行
    uint16_t rowCutBottom = 10;         //图像底部切行
    uint16_t relayLeftFlag=0;
    uint16_t relayRightFlag=0;
	...
	int widthofblack=0;
	    ...
	    widthofblack=0;
            ...
            if (flagStartBlock)                            //起始行做特殊处理
            {
                ...
                int limitWidthBlock = row * 1.05 + 15 ; //首行色块宽度限制（不能太小）
		    ...
		    for (int i = 0; i < counterBlock; i++)
                    {
                        if (startBlock[i] > pointsEdgeLeft[pointsEdgeLeft.size() - 1].y && startBlock[i] < pointsEdgeRight[pointsEdgeRight.size() - 1].y)
			{
                           widthofblack+=startBlock[i]-endBlock[i-1];
				
			    numofblock++;
			}
                    }
		    Widthofblack.push_back(widthofblack);
                    Numofblock.push_back(numofblock);
		...
		if (pointsEdgeLeft[0].y == 0&&pointsEdgeLeft[1].y == 0&&pointsEdgeLeft[2].y == 0) relayRightFlag++;
                if (pointsEdgeRight[0].y == COLSIMAGE - 1&&pointsEdgeRight[1].y == COLSIMAGE - 1&&pointsEdgeRight[2].y == COLSIMAGE - 1) relayLeftFlag++;
    ...
    //单边判定
    void One_Side_Decide()
    {
        relayRightFlag = relayLeftFlag = 0;
        for(auto i: pointsEdgeLeft)
            if (i.y == 0) relayRightFlag++;
        for (auto i : pointsEdgeLeft)
            if (i.y == COLSIMAGE - 1) relayLeftFlag++;
	...
	{
		if(pointsEdgeRight[i].y<eg||pointsEdgeRight[i].y>COLSIMAGE-31)
		{
			eg=pointsEdgeRight[i].y;
			pointsEdgeRight.pop_back();
		}
		else
		{
	...
	{
		if(pointsEdgeLeft[i].y>eg)
		{
			eg=pointsEdgeLeft[i].y;
			pointsEdgeLeft.pop_back();
		}
		else
		{
			eg=pointsEdgeLeft[i].y;
