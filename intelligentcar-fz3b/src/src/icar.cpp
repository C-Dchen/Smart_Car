...
int main(int argc, char const* argv[])
{
    ...
    while (1)
    {
     ...
     // [04] 出库和入库识别与路径规划
        ...
        {
            cout<<"garageRecognition.garageStep="<<garageRecognition.garageStep<<endl;
            if (roadType == RoadType::GarageHandle || roadType == RoadType::BaseHandle)
            {
                ...
                if (garageRecognition.garageRecognition(trackRecognition, resultAI->predictor_results)&&trackRecognition.stdevRight<=10&&trackRecognition.stdevLeft>=150)
                {
        ...
        if (counterRunBegin > 20) //智能车启动延时：前几场图像+AI推理不稳定
        {
           ...
           //  cout<< GarageStep.garageStep <<endl;
            ...
            if (!motionController.params.debug && circlesThis < 3)
                //     &&!freezoneRecognition.freezoneRecognition(trackRecognition, resultAI->predictor_results))//!free()) //调试模式下不控制车辆运动
            {
