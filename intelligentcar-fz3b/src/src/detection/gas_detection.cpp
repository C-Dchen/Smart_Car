    ...
    bool gasStationDetection(TrackRecognition& track, vector<PredictResult> predict)
    {
        ...
        case GasStationStep::GasStationEnable: //[02] 加油站使能
        {
            ...
            else if (_pointNearCone.x > 0.1 && _pointNearCone.x < ROWSIMAGE * 0.4)
            {
                ...
                for (int i = 0; i < predict.size(); i++)
                {
                    if (predict[i].label == LABEL_WRITEONE) //加油站标志检测
                    {
                        counterRec_W1++;
                        break;
                    }

                    if (predict[i].label == LABEL_WRITETWO) //加油站标志检测
                    {
                        counterRec_W2++;
                        break;
                    }
        ...
        case GasStationStep::GasStationCruise: //[04] 巡航使能
        {
            ...
            for (int i = 0; i < predict.size(); i++)
            {
                if (predict[i].label == LABEL_BLUEONE) //加油站标志检测
                {
                ...
                if (predict[i].label == LABEL_BLUETWO) //加油站标志检测
                {
            ...
            counterSession++;
            POINT coneRightDown = searchRightDownCone(pointsCone);                               //右下方锥桶
            if (((coneRightDown.x < ROWSIMAGE / 2 && counterSession > 12) || counterSession > 30) && Label_White == Label_Blue) //右下方锥桶检测完毕
            {
        ...
        case GasStationStep::GasStationExit: //[05] 出站使能
        {
            ...
            {
                gasStationStep = GasStationStep::GasStationNone; //出站结束
                counterRec = 0;
                counterSession = 0;
            }
            else
        ...
        if (gasStationStep == GasStationStep::GasStationNone) //返回加油站控制模式标志
            return false;
        else
            return true;
    }
